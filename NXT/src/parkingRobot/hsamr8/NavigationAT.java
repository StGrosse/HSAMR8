...
public class NavigationAT implements INavigation {
	 ...
	 */
	static final double LEFT_WHEEL_RADIUS = 0.0276; // only rough guess, to be
	 ...
	 */
	static final double RIGHT_WHEEL_RADIUS = 0.0276; // only rough guess, to be
														// measured exactly and
														// maybe refined by
														// experiments
	/**
	 ...
	 */
	static final double WHEEL_DISTANCE = 0.1112; // only rough guess, to be
													// measured exactly and
													// maybe refined by
													// experiments
	...
	/**
	 * pose class containing bundled unprocessed X and Y location and
	 * corresponding heading angle phi
	 */
	 ...
	 */
	// static Line[] map1 = {line0, line1, line2, line3, line4, line5, line6,
	// line7};
	...
	private void calculateLocationLocation() {
		...
		if (this.parkingSlotDetectionIsOn) {
			...
			if (vorwärts) {
				...
				if ((heading > Math.PI * 0.44) && (heading < Math.PI * 0.55) && (currentLine == 0)) {
				...
				if ((heading > Math.PI * 0.85) && (heading < Math.PI * 1.136) && (currentLine == 1)) {
					monitor.writeNavigationComment("enter line2");
					yResult = 0.60;
					angleResult = Math.PI;
					currentLine = 2;
				}
				if ((heading > Math.PI * 1.40) && (heading < Math.PI * 1.67) && (currentLine == 2)) {
				...
				if ((heading > Math.PI * 0.8) && (heading < Math.PI * 1.25) && (currentLine == 3)) {
					monitor.writeNavigationComment("enter line4");
					currentLine = 4;
					yResult = 0.30;
				}
				if ((heading > Math.PI * 0.40) && (heading < Math.PI * 0.60) && (currentLine == 4)) {
					...
					if (parkingSlotDetectionIsOn) {
						...
						// if ((this.pose.getY() > 0.35) && (this.pose.getY() <
						// 0.60)) {
				...
				if ((heading > Math.PI * 0.90) && (heading < Math.PI * 1.10) && (currentLine == 5)) {
					monitor.writeNavigationComment("enter line6");
					yResult = 0.6;
					angleResult = Math.PI;
					currentLine = 6;
					if (parkingSlotDetectionIsOn) {
						// if ((this.pose.getX() <0.25) && (this.pose.getX()
						// >0.10))
						// {
				...
				if ((heading > Math.PI * 1.40) && (heading < Math.PI * 1.60) && (currentLine == 6)) {
					...
					if (parkingSlotDetectionIsOn) {
						...
						xResult = 0;
						angleResult = 1.5 * Math.PI;
						// if ((this.pose.getY() > 0.10) && (this.pose.getY() <
						// 0.58)) {
						if ((this.pose.getHeading() < Math.PI * 1.49) || (this.pose.getHeading() > Math.PI * 1.51)) {
							angleResult = Math.PI * 1.5;
						}
			...
			if (!vorwärts) {
				...
				if ((heading > Math.PI * 0.90) && (heading < Math.PI * 1.10) && (currentLine == 7)) {
					monitor.writeNavigationComment("enter line6");
					yResult = 0.6;
					angleResult = 0.0;
					currentLine = 6;
				...
				if ((heading > -Math.PI * 0.60) && (heading < -Math.PI * 0.40) && (currentLine == 6)) {
					monitor.writeNavigationComment("enter line5");
					xResult = 0.3;
					currentLine = 5;
					angleResult = -0.5 * Math.PI;
					if (parkingSlotDetectionIsOn) {
						...
						// if ((this.pose.getY() > 0.35) && (this.pose.getY() <
						// 0.60)) {
						if ((heading > -Math.PI * 0.51) || (heading < -Math.PI * 0.49)) {
							angleResult = -Math.PI * 0.5;
						}
				...
				if ((heading > Math.PI * -0.1) && (heading < Math.PI * 1.10) && (currentLine == 5)) {
					monitor.writeNavigationComment("enter line6");
					yResult = 0.3;
					angleResult = 0.0;
					currentLine = 4;
				...
				if ((heading > -Math.PI * 0.1) && (heading < Math.PI * 0.1) && (currentLine == 3)) {
					monitor.writeNavigationComment("enter line4");
					currentLine = 2;
					yResult = 0.30;
				}
			...
			// if ((this.pose.getHeading() >Math.PI * 1.40) &&
			// (this.pose.getHeading() <Math.PI * 1.67) && (currentLine== 7)) {
			if (currentLine == 7) {
	...
	private void detectParkingSlot() {
		...
		case 0:
			if (this.pose.getX() < 1.75) {
				...
				if ((parkenSlotSuch0 == false) && (this.frontSideSensorDistance > 280)) {
					...
					// backBoundaryPosition = new
					// Point(this.pose.getX()+parkenGutLength1,
					// this.pose.getY());
		...
		case 1:
			if ((this.pose.getY() > 0.05) && (this.pose.getY() < 0.6)) {
				...
				if ((parkenSlotSuch1 == true) && (this.frontSideSensorDistance < 220)) {
					...
					// frontBoundaryPosition = new
					// Point(this.pose.getX(),this.pose.getY());
		...
		case 4:
			if ((this.pose.getX() < 1.27) && (this.pose.getX() > 0.3)) {
				...
				if ((parkenSlotSuch4 == false) && (this.backSideSensorDistance > 380)) {
					...
					// backBoundaryPosition = new Point(this.pose.getX() ,
					// this.pose.getY());
		...
		if (signal == true) {
			if ((this.frontBoundaryPosition != null) && (this.backBoundaryPosition != null)) {
				frontparkPosition = Math.abs(this.frontBoundaryPosition.getX())
						+ Math.abs(this.frontBoundaryPosition.getY());
				backparkPosition = Math.abs(this.backBoundaryPosition.getX())
						+ Math.abs(this.backBoundaryPosition.getY());
				// frontparkPosition =
				// Math.abs(this.frontBoundaryPosition.getX());
				// backparkPosition =
				// Math.abs(this.backBoundaryPosition.getX());
			...
			if (parkingSlotLength >= 0.45) {
				...
				/*
				 * this.monitor.writeNavigationComment("X_BACK "+Slots[0].
				 * getBackBoundaryPosition().x);
				 * this.monitor.writeNavigationComment("Y_BACK "+Slots[0].
				 * getBackBoundaryPosition().y);
				 * this.monitor.writeNavigationComment("X_FRONT "+Slots[0].
				 * getFrontBoundaryPosition().x);
				 * this.monitor.writeNavigationComment("Y_FRONT "+Slots[0].
				 * getFrontBoundaryPosition().y);
				 * this.monitor.writeNavigationComment("Parken_ID "+Slots[0].
				 * getID());
				 * this.monitor.writeNavigationComment("STATUS "+Slots[0].
				 * getStatus()); this.monitor.writeNavigationComment(
				 * "TEST:  grose des Arrays:"+Slots.length);
				 */
				...
				ParkingSlot getParkingSlots = new ParkingSlot(parken_ID, backBoundaryPosition, frontBoundaryPosition,
						status, measurementQuality);
				parken_ID--;
			}
			/*
			 * //aktualiseung if(Slots.length>=2){ for(int
			 * i=0;i<Slots.length;i++){
			 * if(sameSlot(Slots[i],Slots[Slots.length-1])){ //Slot[neu] besser
			 * als Slot[alt] if(compareSlot(Slots[Slots.length-1],Slots[i])){ //
			 * Slots[Slots.length-1].setID(i); Slots[i]=Slots[Slots.length-1];
			 * slotList.remove(Slots.length-1); Slots = new
			 * ParkingSlot[slotList.size()]; Slots = slotList.toArray(Slots); }
			 * else{ slotList.remove(Slots.length-1); Slots = new
			 * ParkingSlot[slotList.size()]; Slots = slotList.toArray(Slots); }
			 * } } }
			 */
			 ...
			 * ParkingSlot[slotList.size()]; Slots = slotList.toArray(Slots);
			 * 
			 * } else{ slotList.remove(0); Slots = new
			 * ParkingSlot[slotList.size()]; Slots = slotList.toArray(Slots);
			 * 
			 * }
			 * 
			 * 
			 * 
			 * this.monitor.writeNavigationComment(
			 * "NumberSlot:"+slotList.size());
			 * this.monitor.writeNavigationComment("bestenSlot_ID:"+Slots[0].
			 * getID()); }
			 */
	...
	public boolean sameSlot(ParkingSlot x, ParkingSlot y) {
		if ((Math.abs((x.getBackBoundaryPosition().getX()) - y.getBackBoundaryPosition().getX()) < SameSlotDifference)
				&& ((Math.abs(
						x.getBackBoundaryPosition().getY() - y.getBackBoundaryPosition().getY()) < SameSlotDifference))
				|| ((Math.abs(x.getFrontBoundaryPosition().getX())
						- y.getFrontBoundaryPosition().getX()) < SameSlotDifference)
						&& ((Math.abs(x.getFrontBoundaryPosition().getY()
	...
	public boolean compareSlot(ParkingSlot x, ParkingSlot y) {
		...
		double frontparkPositionX = Math.abs(x.getFrontBoundaryPosition().getX())
				+ Math.abs(x.getFrontBoundaryPosition().getY());
		double backparkPositionX = Math.abs(x.getBackBoundaryPosition().getX())
				+ Math.abs(x.getBackBoundaryPosition().getY());
		double frontparkPositionY = Math.abs(y.getFrontBoundaryPosition().getX())
				+ Math.abs(y.getFrontBoundaryPosition().getY());
		double backparkPositionY = Math.abs(y.getBackBoundaryPosition().getX())
				+ Math.abs(y.getBackBoundaryPosition().getY());
	...
	/*
	 * Die Qualitaet von Parkenluecken(die Genauigkeit) Array wahrenWert[] means
	 * true data Die Einheit ist cm
	 * 
	...
	public int qualitySlot(double parkingSlotLength, int parkenSlot_ID) {
		...
		wahrenWert[2] = 0.45;
		wahrenWert[3] = 0.64;
		// double frontparkPosition =
		// Math.abs(x.getFrontBoundaryPosition().getX()) +
		// Math.abs(x.getFrontBoundaryPosition().getY());
		// double backparkPosition =
		// Math.abs(x.getBackBoundaryPosition().getX()) +
		// Math.abs(x.getBackBoundaryPosition().getY());
		// double parkingSlotLength = Math.abs(frontparkPosition -
		// backparkPosition);
		switch (parkenSlot_ID) {
		case 0:
			if (Math.abs(parkingSlotLength - wahrenWert[0]) <= 0.15) {
				measurementQuality = 3;
			} else if (Math.abs(parkingSlotLength - wahrenWert[0]) < 0.25
					&& (Math.abs(parkingSlotLength - wahrenWert[0]) > 0.15)) {
		...
		case 1:
			if (Math.abs(parkingSlotLength - wahrenWert[1]) <= 0.15) {
				measurementQuality = 3;
			} else if (Math.abs(parkingSlotLength - wahrenWert[1]) < 0.25
					&& (Math.abs(parkingSlotLength - wahrenWert[1]) > 0.15)) {
				measurementQuality = 2;
			} else {
				measurementQuality = 1;
			}
		...
		case 2:
			if (Math.abs(parkingSlotLength - wahrenWert[2]) <= 0.15) {
				measurementQuality = 3;
			} else if (Math.abs(parkingSlotLength - wahrenWert[2]) < 0.25
					&& (Math.abs(parkingSlotLength - wahrenWert[2]) > 0.15)) {
				measurementQuality = 2;
			} else {
				measurementQuality = 1;
			}
		...
		case 3:
			if (Math.abs(parkingSlotLength - wahrenWert[3]) <= 0.15) {
				measurementQuality = 3;
			} else if (Math.abs(parkingSlotLength - wahrenWert[3]) < 0.25
					&& (Math.abs(parkingSlotLength - wahrenWert[3]) > 0.15)) {
				measurementQuality = 2;
			} else {
				measurementQuality = 1;
			}
